...
- **Full** — Cesare drawer state that is a larger floating panel, NOT a viewport takeover. It anchors to the right edge at full height with a capped width (`clamp(420px, 56vw, 720px)`), so the open document stays live and visible behind/beside it. The editor never reflows and is never fully hidden (Notion live-doc model).
- **Cesare Drawer** — Cesare-only floating panel anchored bottom-right (Notion AI chat pattern). States: `closed | peek | expanded | expanded-split | full`. **Never** anchors as a column that pushes the editor — Cesare always floats. `expanded` = floating 480×640 sub-window; `expanded-split` = the transient state used in the SplitDrawer cross-flow (drawer narrows so the SplitDrawer can co-exist on the right); `full` = larger right-anchored full-height floating drawer, width-capped so the editor stays visible. AppShell's `body[data-cesare]` flag persists only `closed` and `expanded` — the other three states are transient. Owns: header (agent + context tags + session selector + window-control icons), body (messages + Step Blocks), sticky footer (quick prompts + composer with file/scope chips). The Drawer is the **only** Cesare surface in the app — there is no second chat container anywhere else.
...
- **AppShell Shell State** — `full` (rail 240px locked open + topstrip + dock) | `collapsed` (rail hidden + hamburger top-left always visible; click hamburger → rail slides in as overlay; click outside / ESC / hamburger again → closes; NO hover-reveal) | `focus` (rail + topstrip + dock hidden, hamburger hidden). Toggled via `⌘\` (Notion shortcut) for full↔collapsed and `⌃⌥F` for focus. Persisted per user.

- **Rail overlay (collapsed mode)**: when `data-shell="collapsed"`, the rail is hidden by default and a hamburger button (`☰`) sits at top-left coordinates (`top:12px, left:12px`), always visible while collapsed. Clicking the hamburger toggles `body[data-rail-overlay]` between `open` and absent; the rail CSS reads that attribute and slides in as an overlay on top of the editor (`z-index:320`, `transform: translateX(0)`). The editor never reflows because the rail is `position: fixed` while collapsed — it sits above the grid. Dismiss paths: hamburger again, click outside the rail, press ESC, or click the `«` close button in the rail's top-right. A `»` "lock open" chip is also surfaced; clicking it switches the shell back to `full` and unmounts the overlay. There is no hover-based reveal or auto-close. The hamburger position never changes between open and closed overlay states.
...
| `full`     | right edge, full height       | `clamp(420px, 56vw, 720px) × 100vh` (width-capped) | rail+topstrip+dock stay visible; editor's left portion stays live behind/beside the drawer | slide-in from right                                  |
