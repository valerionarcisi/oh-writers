...
> **Resolution (2026-06-01):** all GAP rows below are now CLOSED. Schedule, budget,
> and shooting-plan each have CSV + PDF server functions, a dedicated hook, and wired
> dock buttons:
>
> - `schedule/server/schedule-export.server.ts` → `exportScheduleCsv` + `exportSchedulePdf`, hook `useExportSchedule`, SchedulePage (Esporta = CSV, Stampa = PDF).
> - `budget/server/budget-export.server.ts` → `exportBudgetCsv` + `exportBudgetPdf`, `ExportBudgetModal`, hook `useExportBudget`.
> - `shooting-plan` → `useExportShotList` (CSV + PDF), `ShootingPlanPage` passes `onExport`/`onPrint`.
>   The table below reflects the original 2026-05-17 audit and is kept for history.
...
All export triggers are consistent in terms of _where_ they live:
