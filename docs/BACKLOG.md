...
-D. **[BUG-N64] `?versions=…&peek=cesare` spacca la pagina** — ✅ FIXED (branch `fix/n64-route-combo-failclosed`, non mergiato). Resolver fail-closed in AppShell: una sola lane ausiliaria attiva, precedenza Versions > Cesare peek > preview; la main lane non collassa più. Verificato live (chrome) + E2E [BUG-N64]. `docs/BUGS.md`.
...
30. **[N-62] CI `e2e-chromium` has NEVER been green on the GitHub runner** (local: green). 2026-06-11 run: 380 pass / 14 hard / 26 flaky in 53m. Failure classes are runner-specific: 1.5m timeouts (teams ×3, pdf-import OHW-091/092, shooting-plan), keyboard-shortcut tests (⌘+Number ×4, Alt+C — suspect Meta-vs-Control on Linux), screenplay-authoring S03–S06, locations OHW-379. Needs its own front: reproduce on a Linux container or instrument the runner; until then the job is advisory — the push gate (`ci:repro`) mirrors only the mock-ui job, which IS green as of `a236a7c5`.
    - **KEYBOARD class FIXED 2026-06-13 (`c747f890`, on main):** the editor keymap is `Mod-` (Cmd/macOS, Ctrl/Linux); tests pressed `Meta+1/3/4/6` + `Meta+z` hardcoded → green on the Mac box, red on the Linux runner. Switched to Playwright `ControlOrMeta+` (screenplay-editor-ux, scene-heading, cesare-inline-edits), 24/24 still green locally. This should clear the ⌘+Number / undo reds on the runner. `shell-collapse`'s `Meta+Backslash` left as-is (AppShell accepts metaKey||ctrlKey).
    - **STILL OPEN:** the runner TIMEOUT class (teams, pdf-import OHW-091/092, shooting-plan, screenplay-authoring S03–S06, locations OHW-379) — not reproducible on the fast Mac box; needs the Linux runner (raise per-test timeouts/retries or instrument a Linux container). Also new E2E added 2026-06-13 (`c514baf6`): `screenplay-export.spec` [OHW-N63a/c/d] lock the WYSIWYG export fixes (no blank page, bold heading via fontName, dialogue indented) — green locally.
