...
- **N-32** BASSO (a11y / touch + i18n) — **Screenplay focus mode has no clickable ENTER affordance and the in-editor "Exit Focus" button is hardcoded English.** Per Spec 55a focus is intentionally not a TopBar action: entering is keyboard-only (`Ctrl/Cmd+Shift+F`), exiting uses the in-editor button. On a touch/iPad PWA (a platform-reach target) there is no way to enter focus mode without a keyboard; and the "Exit Focus" / "Exit focus mode" strings (`ScreenplayEditor.tsx`, pre-existing on `main`) violate the IT-copy rule. Fix: a small clickable focus toggle (editor-local, not the TopBar) + move the label to `screenplay.*` keys. Found in the A5 review; out of A5 chrome scope.
