 ...
 *
 * - 542: the RecapStrip renders the day cost for the active scene.
 ...
 * - 543: The user asks Cesare to estimate scene cost. The scripted
...
test.describe("[Spec 34] Cesare Agentic — Breakdown", () => {
  test("[542] RecapStrip renders the day cost and category chips", async ({
    authenticatedPage,
  }) => {
  ...
  test("[543] Cesare estimates scene cost via estimate_scene_cost tool", async ({
    authenticatedPage,
  }) => {
    await navigateToBreakdown(authenticatedPage, TEAM_PROJECT_ID);
