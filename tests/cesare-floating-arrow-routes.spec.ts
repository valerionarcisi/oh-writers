...
test.describe("[046] Cesare floating ↗ routes to session detail", () => {
  test("↗ navigates to /sessions/:id and dismisses the floating drawer", async ({
    ...
    // The drawer resolves its active session from the project sessions query;
    // wait for that to populate so ↗ has a target (the selector trigger shows
    // the active session title once it is known).
    await expect(page.getByLabel("Seleziona sessione Cesare")).toBeVisible({
      timeout: 10_000,
    });
