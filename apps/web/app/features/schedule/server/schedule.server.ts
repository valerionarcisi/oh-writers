...
import {
  ...
  type LocationRequirementStatus,
...
// ─── Day location warnings ────────────────────────────────────────────────────
//
// Pure helper: given the raw location requirements joined to scenes for a
// shooting day, return the subset whose status is NOT "confirmed". The schedule
// page renders these as soft warning banners. Cesare's suggest_reorder can use
// the same data to bias the suggestion away from days with pending locations.

export interface DayLocationWarning {
...
export interface DayLocationRow {
  ...
  readonly requirementName: string;
  readonly requirementStatus: LocationRequirementStatus;
}

export const buildDayLocationWarnings = (
  rows: ReadonlyArray<DayLocationRow>,
): DayLocationWarning[] => {
  const seen = new Set<string>();
  const warnings: DayLocationWarning[] = [];
  for (const row of rows) {
    if (
      row.requirementStatus === "confirmed" ||
      row.requirementStatus === "locked"
    ) {
      continue;
    }
    const key = `${row.sceneId}:${row.requirementName}`;
    if (seen.has(key)) continue;
    seen.add(key);
    warnings.push({
      sceneId: row.sceneId,
      sceneNumber: row.sceneNumber,
      location: row.sceneLocation,
...
export const getDayLocationWarnings = createServerFn({ method: "GET" })
  .validator(z.object({ dayId: z.string().uuid() }))
  .handler(async ({ data }) => {
    ...
    const result = await ResultAsync.fromPromise(
      db.query.shootingDays
        .findFirst({ where: eq(shootingDays.id, data.dayId) })
        .then((r) => r ?? null),
      (e) => new DbError("getDayLocationWarnings/findDay", e),
    )
      .andThen((day) => {
        if (!day) return errAsync(new ShootingDayNotFoundError(data.dayId));
        return ResultAsync.fromPromise(
          db.query.schedules.findFirst({
            where: eq(schedules.id, day.scheduleId),
          }),
          (e) => new DbError("getDayLocationWarnings/findSchedule", e),
        ).map((s) => ({ day, schedule: s }));
      })
      .andThen(({ day, schedule }) => {
        if (!schedule) return errAsync(new ShootingDayNotFoundError(day.id));
        return requireProjectAccess(db, schedule.projectId, "view").map(() => ({
          day,
        }));
      })
      .andThen(({ day }) =>
        ResultAsync.fromPromise(
          db
            .select({
              ...
              requirementStatus: locationRequirements.status,
            })
            .from(strips)
            .innerJoin(scenes, eq(strips.sceneId, scenes.id))
            .innerJoin(
            ...
            .where(eq(strips.shootingDayId, day.id)),
          (e) => new DbError("getDayLocationWarnings/query", e),
        ).map((rows) =>
          buildDayLocationWarnings(
            rows.map((r) => ({
              sceneId: r.sceneId,
              sceneNumber: r.sceneNumber,
              sceneLocation: r.sceneLocation,
              requirementName: r.requirementName,
              requirementStatus:
                r.requirementStatus as LocationRequirementStatus,
            })),
          ),
        ),
      );
...
export const dayLocationWarningsQueryOptions = (dayId: string) => ({
  queryKey: ["schedule", "day", dayId, "location-warnings"] as const,
  queryFn: () => getDayLocationWarnings({ data: { dayId } }),
});
