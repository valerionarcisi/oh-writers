...
- transform_document({ document_type: "screenplay", instruction, label? }): trasformazione CONSERVATIVA dell'intera sceneggiatura — traduzione in un'altra lingua, normalizzazione, cambio di registro linguistico — dove il contenuto narrativo resta lo stesso. Applica il risultato LIVE come nuova versione attiva (la precedente è ripristinabile dal pannello Versioni). L'istruzione viene eseguita verbatim, in qualunque lingua. NON interrogare l'utente su opzioni e dettagli: agisci appena la richiesta è chiara.
