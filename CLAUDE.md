...
- **Never write code in Italian** — every identifier, comment, log message and internal error message MUST be in English. This is unchanged and non-negotiable.
- **English is the product's default/source language (decided 2026-09-10)** — Oh Writers is multilingual, English-first: every UI copy string is authored in English in the i18n catalogue first, then translated to Italian, never the other way around. This inverts the product's original IT-only framing — the app started Italian-only and later grew a real i18n layer (`packages/domain/src/i18n/`, `resolveLocale.server.ts` already defaults unauthenticated/no-header requests to `Locales.EN`); going forward EN is the source of truth and IT is a translation target, not the default the code falls back to describing. UI copy lives in the i18n catalogue, never as a literal in a component — a comment `// gestisce errore` is as much a violation as a function named `caricaSceneggiatura`, same as before.
