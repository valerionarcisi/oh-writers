...
const PAGE_TRACE_VIEW_REGISTRY = new Map<
  TargetPageKind,
...
function markerGlyphStyle(
  kind: TraceMarkerKind,
  base: string | undefined,
): string {
  const safeBase = base ?? "";
  if (kind === "delete")
    return `${safeBase} ${styles.markerGlyphRemove ?? ""}`.trim();
  if (kind === "replace")
    return `${safeBase} ${styles.markerGlyphReplace ?? ""}`.trim();
  return safeBase;
}
...
function TimelineButton({
  ...
  const { buttonProps } = useButton(
    {
      onPress,
      "aria-label": `Vai a ${marker.anchor} (${MARKER_LABELS[marker.kind]})`,
    },
    ref,
  );
  return (
...
function MarkerBlock({
  ...
  const blockClass = useMemo(() => {
    if (marker.kind === "delete")
      return `${styles.markerBlock} ${styles.markerBlockRemove}`;
    if (marker.kind === "append")
      return `${styles.markerBlock} ${styles.markerBlockAdd}`;
      ...
      <div ref={previewRef} className={styles.preview}>
        <div className={styles.previewHeader}>
          <span className={styles.previewBadge}>
            {pageRef.scope
              ? `${KIND_LABELS[pageRef.kind]} · ${pageRef.scope}`
              : KIND_LABELS[pageRef.kind]}
          </span>
